...
do_ioi = False
if do_ioi:
    ioi_task = IOITask(batch_size=10, tokenizer=tokenizer, device=device, prep_acdcpp=True)
    ioi_task.set_logit_diffs(model, batched=True)
...
for forget_sport in ['all']: # ['all']:#
    ...
    for name, task in zip(["sports"], [sports_task]):
        ...
        torch.cuda.empty_cache()
        gc.collect()
        ap_graph = ap_localizer.get_ap_graph(batch_size=4)
        torch.cuda.empty_cache()
        gc.collect()
        with open(f"models/{save_model_name}_{name}_{forget_sport}_ap_graph.pkl", "wb") as f:
            pickle.dump(dict(ap_graph), f)

        print("Starting Causal Tracing")
        model.eval() # Don't need gradients when doing ct task
        ct_graph = ct_localizer.get_ct_mask(batch_size=4, gemma2=True)
        model.train()
