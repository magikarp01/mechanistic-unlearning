...
assert args.forget_split == "first_64_partitioned_unsplit", "Sequential Editing only implemented for first 64 facts partitioned into 4 groups of 16"
regular_forget_split = "first_64_unsplit"
...
partition_names = ["0_16", "16_32", "32_48", "48_64"]
for partition in range(4):

    print(f"==========Partition {partition}, {partition_names[partition]}==========")
    forget_split_file_name = partition_names[partition]
    ...
    print(f"Manual param count for partition {partition}: ", manual_param_count)
    ...
    partition_forget_split = f"first_64_partition_{partition}_unsplit"
    print(f"Current partition forget split: {partition_forget_split}")

    eval_forget_kwargs = {"forget_split": regular_forget_split, "maintain_split": None, "model_type": model_type}
    partition_forget_kwargs = {"forget_split": partition_forget_split, "maintain_split": None, "model_type": model_type}
    maintain_kwargs = {"forget_split": partition_forget_split, "maintain_split": "split", "model_type": model_type}
    inject_fact = args.inject_fact
    ...
    if inject_fact:
        facts_injection = CounterFactTask_Injection(batch_size=train_batch_size, tokenizer=tokenizer, device=device, inject_fact=inject_fact, **partition_forget_kwargs)
        ...
        facts_1mp = CounterFactTask(batch_size=train_batch_size, tokenizer=tokenizer, device=device, criterion="log_1_minus_p", **partition_forget_kwargs)
    ...
    forget_fact_eval = CounterFactTask_Injection(batch_size=eval_batch_size, tokenizer=tokenizer, device=device, criterion="cross_entropy", **eval_forget_kwargs)
    ...
    print(f"{facts_injection.train_df.index=}")
    print(f"{maintain_facts.train_df.index=}")
    print(f"{forget_fact_eval.train_df.index=}")
    ...
    with open(f"models/{localization_model_name}_counterfact_ap_graph_{forget_split_file_name}.pkl", "rb") as f:
        ap_graph = pickle.load(f)
    with open(f"models/{localization_model_name}_counterfact_ct_graph_{forget_split_file_name}.pkl", "rb") as f:
    ...
    for local_epoch in pbar:
        epoch = local_epoch + n_epochs * partition
        # Sample batches
        # Reset grad
        optimizer.zero_grad()
        ...
        if local_epoch % evaluate_every == 0 or local_epoch == n_epochs - 1:
        ...
        if (deep_evaluate_every is not None and epoch % deep_evaluate_every == 0) or local_epoch == n_epochs - 1:
            if do_adversarial_evals:
                ...
                adv_evals = adversarial_counterfact_eval(model, model_type=model_type, batch_size=eval_batch_size, 
                    forget_task_init_kwargs=eval_forget_kwargs, 
...
if args.do_softprompt_evals:
    ...
    tokenizer.padding_side = "right"
    if "unsplit" in regular_forget_split:
        relearn_forget_split = regular_forget_split.replace("unsplit", "split")
        print(f"Original forget split is {regular_forget_split}, relearning with {relearn_forget_split}")
    else:
        print("Why is the forget_split originally train-test-splitted? This probably shouldn't be happening")
        relearn_forget_split = regular_forget_split
