...
model = HookedTransformer.from_pretrained(MODEL_NAME, default_padding_side="left", device=device, dtype=DTYPE)
# model = model.half()
...
with torch.no_grad():
    ...
    mc_edit_over_orig_acc /= total
    wandb.log({
        "final_mc_edit_over_orig_acc": mc_edit_over_orig_acc, 
        "final_mc_edit_acc": mc_edit_acc, 
        "final_mc_orig_acc": mc_orig_acc
    })
# %%
