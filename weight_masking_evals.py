...
# %%
os.environ['HF_TOKEN'] = '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'
model_type = "gemma"
...
with torch.autocast(device_type="cuda"), torch.set_grad_enabled(False):
    for localization_type in localization_types:
        ...
        mask = torch.load(
            f"results/{model_name.replace('/', '_')}-counterfact-{localization_type}-{SUFFIX}.pt",
            map_location="cuda"
        )
        ...
        for num_weights in [0, 1_000_000, 2_000_000, 3_000_000, 4_000_000, 5_000_000, 6_000_000]:
            if num_weights > len(sorted_nonzero):
                num_weights = len(sorted_nonzero) - 1
            ...
            # Load Model
            model = load_model()
            mask = torch.load(f"results/{model_name.replace('/', '_')}-counterfact-{localization_type}-{SUFFIX}.pt", map_location="cuda")
            ...
            gc.collect()
            torch.cuda.empty_cache()
            with open(f"results/{model_name.replace('/', '_')}-{localization_type}-results-counterfact-{str_num_weights}-backup-{SUFFIX}.json", "w") as f:
                json.dump(results[localization_type][str_num_weights], f, indent=2)

        with open(f"results/{model_name.replace('/', '_')}-{localization_type}-results-counterfact-{SUFFIX}.json", "w") as f:
            json.dump(results[localization_type], f, indent=2)

with open(f"results/{model_name.replace('/', '_')}-counterfact-{SUFFIX}-results.json", "w") as f:
